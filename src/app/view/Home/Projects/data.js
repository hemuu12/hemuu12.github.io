exports.featuredProjects = [
	{
		name: "BeautyBebo",
		description:
			"Beauty Bebo Products on your fingure tips. Just imagine your most favorite product and instruct your finger tips to order it on beautybebo and soon it will be delivered to your doorstep.",
		tags: ["HTML", "CSS", "Javscript"],
		deployedLink: "https://starlit-fenglisu-cc481f.netlify.app/",
		gitURL: "https://github.com/hemuu12/Beauty-Bebo-Clone",
	},

	{
		name: "Yoox",
		description:
			"New Arrivals Every Week to Reinvigorate your Wardrobe, Make the Most of it Now. Shop the Latest Designer Savings Now! Secure Payment. Expert Customer Care. A Wide Array of Brands. Highlights: login signup.",
		tags: ["HTML", "CSS", "Javscript","React"],
		gitURL: "https://github.com/hemuu12/My-Yoox-",
		deployedLink: "https://my-app-hemuu12.vercel.app/",
	},

	{
		name: "Socialhub",
		description:
			"Socialhub it is an website of all Socialmedia post Help website where you can make your awesome post and schedule your post for socailmedia where you have to post.",
		tags: ["HTML", "CSS", "Javscript","React","React-Redux"],
		deployedLink: "https://sociahubb.netlify.app/",
		gitURL: "https://github.com/VinayRinait/Buffer_Website_Clone",
	},
	{
		name: "YoutubeClone",
		description:
			"Youtube Clone where you can Watch videos without Ads and enjoy all the videos there.",
		tags: ["HTML", "CSS", "Javscript"],
		deployedLink: "https://lively-moonbeam-8eb446.netlify.app/",
		gitURL: "https://github.com/hemuu12/youtube-clone",
	},
	{
		name: "Glowup",
		description:
			"Glowup is a Cosmatic Selling Website here we have many brands of Products are available.",
		tags: ["HTML", "CSS", "Javscript", "Redux", "MongoDb", "Expressjs", "Nodejs"],
		deployedLink: "https://glowup-com.netlify.app/",
		gitURL: "https://github.com/gk072745/nifty-wave-512",
	},
];











// exports.ugCourses = [
// 	{
// 		name: "CSE-474-Pattern-Recognition",
// 		description:
// 			"Implementation of various pattern recognition techniques. Implemented naive-bayes-classifier, basic perceptron algorithms, neural networks etc.",
// 		gitURL: "https://github.com/Waqar-107/CSE-474-Pattern-Recognition",
// 		tags: ["Python", "Jupyter Notebook", "Naive-Bayes", "Perceptron", "Neural Network"],
// 	},

// 	{
// 		name: "CSE-412-Simulation-and-Modelling",
// 		description: "Simulation of different systems like M/M/K queue, job-shop model etc.",
// 		gitURL: "https://github.com/Waqar-107/CSE-412-Simulation-and-Modelling",
// 		tags: ["Python"],
// 	},

// 	{
// 		name: "CSE-410-Computer-Graphics",
// 		description:
// 			"Use of OpenGL, implementing how functions of OpenGL works using matrices and implementing a given scene with the concepts of ray tracing.",
// 		gitURL: "https://github.com/Waqar-107/CSE-410-Computer-Graphics",
// 		tags: ["C", "C++", "OpenGL", "Ray Tracing"],
// 	},

// 	{
// 		name: "CSE-406-Computer-Security",
// 		description:
// 			"Transposition cipher and  DES were implemented. Also buffer overflow, XSS and CSRF attacks were performed.",
// 		gitURL: "https://github.com/Waqar-107/CSE-406-Computer-Security",
// 		tags: ["C++", "Python"],
// 	},

// 	{
// 		name: "CSE-322-Computer-Networks",
// 		description:
// 			"Use of SMTP and HTTP protocol, DVR implementation, RDT simulation, error correction using CRC checksum were implemented. Also experimented with NS2.",
// 		gitURL: "https://github.com/Waqar-107/CSE-322-Computer-Networks",
// 		tags: ["C", "C++", "Tcl", "Shell", "NS2"],
// 	},

// 	{
// 		name: "CSE-318-Artificial-Intelligence",
// 		description:
// 			"TSP, n-puzzle problem, missionary and cannibal problem, mancala game and csp were implemented.",
// 		gitURL: "https://github.com/Waqar-107/CSE-318-Artificial-Intelligence",
// 		tags: ["C", "C++", "Python"],
// 	},

// 	{
// 		name: "CSE-314-Operating-System",
// 		description:
// 			"Basic shell programming, Pthreads, Semaphores and Mutex were done. xv6 was selected to experiment with. Adding system calls, implementing socket api and paging for xv6 were done.",
// 		gitURL: "https://github.com/Waqar-107/CSE-314-Operating-System",
// 		tags: ["C", "C++", "Python", "xv6", "Shell"],
// 	},

// 	{
// 		name: "CSE-307-Software-Engineering",
// 		description: "Various design patterns were taught.",
// 		gitURL: "https://github.com/Waqar-107/Design-Pattern",
// 		tags: ["Java"],
// 	},

// 	{
// 		name: "CSE-310-Compiler",
// 		description:
// 			"A subset of the gcc compiler. It was made through four iterations. First a data structure called symbolTable was made, using lex a code was tokenized, using yacc the grammer was written and finally converted into 8086-assembly code.",
// 		gitURL: "https://github.com/Waqar-107/CSE-310-Compiler",
// 		tags: ["C", "C++", "Yacc", "Lex", "Shell"],
// 	},

// 	{
// 		name: "CSE-300-Technical-Writing-and-Presentation",
// 		description:
// 			"LaTex was taught, also basics of techincal writings and presentation were taught.",
// 		gitURL: "https://github.com/Waqar-107/CSE-300-Technical-Writing-and-Presentation",
// 		tags: ["LaTex"],
// 	},

// 	{
// 		name: "CSE-316-Microprocessors-Microcontrollers-and-Embedded-Systems.",
// 		description:
// 			"Assembly language for intel 8086 microprocessor were taught. Also ATMega32 were used to experiment.",
// 		gitURL:
// 			"https://github.com/Waqar-107/CSE-316-Microprocessors-Microcontrollers-and-Embedded-Systems",
// 		tags: ["Assembly", "C", "C++", "ATMega32"],
// 	},

// 	{
// 		name: "CSE-208-Data-Structure-and-Algorithm-2",
// 		description:
// 			"Prims, Kruskals, Bellman-Ford, Floyd-Warshall algorithm, Ford-Fulkerson Method were implemented. Also Red-Black tree, Branch and Bound, Hashing techniques, Dynamic Programmings were done.",
// 		gitURL: "https://github.com/Waqar-107/CSE-208-Data-Structure-and-Algorithm-2",
// 		tags: ["C++"],
// 	},

// 	{
// 		name: "CSE-215-Database",
// 		description: "SQL for Oracle were taught.",
// 		gitURL: "https://github.com/Waqar-107/CSE-215-Database",
// 		tags: ["PLSQL", "Oracle-11g"],
// 	},

// 	{
// 		name: "CSE-218-Numerical-Methods",
// 		description:
// 			"Various numerical methods such as Bisection, Gauss-Jordan, Runge-Kutta were implemented.",
// 		gitURL: "https://github.com/Waqar-107/CSE-218-Numerical-Methods",
// 		tags: ["Matlab"],
// 	},

// 	{
// 		name: "CSE-204-Data-Structure-and-Algorithm-1",
// 		description:
// 			"Data structures such as arraylist, linked-list, binary search tree, heap, graph were implemented. Also divide and conquer and greedy techniques were taught.",
// 		gitURL: "https://github.com/Waqar-107/CSE-204-Data-Structure-and-Algorithm-1",
// 		tags: ["C++"],
// 	},

// 	{
// 		name: "CSE-108-Object-Oriented-Programming-Language",
// 		description: "C++ and Java were taught.",
// 		gitURL: "https://github.com/Waqar-107/CSE-108-Object-Oriented-Programming-Language",
// 		tags: ["C++", "Java"],
// 	},
// ];
// exports.projects = [
// 	{
// 		name: "ShashthoSheba",
// 		description:
// 			"A telemedicine app for online consultation. The project is under the supervision of Prof. Dr. A.B.M Alim Al Islam(Razi) sir. I worked in the backend which is developed in Node.js and MongoDB. The app has been developed in Flutter.",
// 		tags: ["Node.js", "MongoDB", "Flutter", "AWS"],
// 	},

// 	{
// 		name: "DoS-Attack-to-The-DNS-Server",
// 		description:
// 			"This is a console based program that can flood the DNS server by continuously sending dns queries. The code was tested on Bind9 DNS server. The os was seed-labs ubuntu 16.04.",
// 		gitURL: "https://github.com/Waqar-107/DoS-Attack-to-The-DNS-Server",
// 		tags: ["Python", "Bind9", "Shell"],
// 	},

// 	{
// 		name: "Centre for Energy Studies",
// 		description:
// 			"An webapp for the Centre for Energy Studies, BUET. The frontend is made using React js and the backend using Node.js and MongoDB. There's also an admin panel to update the data.",
// 		tags: ["React js", "Node.js", "MongoDB", "MERN-Stack"],
// 		deployedLink: "http://ces.buet.ac.bd/",
// 	},

// 	{
// 		name: "ChessMaestro",
// 		description: "A multiplayer chess game implemented using java.",
// 		gitURL: "https://github.com/Waqar-107/ChessMaestro",
// 		tags: ["Java", "JavaFx"],
// 	},

// 	{
// 		name: "Nymeria",
// 		description:
// 			"An accelerometer based hand gesture controlled robot. This was made during my level-3, term-1 as the microcontroller project.",
// 		gitURL: "https://github.com/Waqar-107/Nymeria",
// 		tags: ["C", "C++", "ATMega32"],
// 	},

// 	{
// 		name: "Rail-Rush",
// 		description:
// 			"A railway management software. Ticket management, train schedule maintaining can be done. Also tender management has been implemented both for the autority and the bidder.",
// 		gitURL: "https://github.com/Waqar-107/Rail-Rush",
// 		tags: ["HTML", "CSS", "Bootstrap", "PHP", "JavaScript", "Oracle-11g"],
// 	},

// 	{
// 		name: "C compiler",
// 		description:
// 			"A subset of the gcc compiler that was made during my level-3, term-1. It was made through four iterations. First a data structure called symbolTable was made, using lex a code was tokenized, using yacc the grammer was written and finally converted into 8086-assembly code.",
// 		gitURL: "https://github.com/Waqar-107/CSE-310-Compiler",
// 		tags: ["C", "C++", "Yacc", "Lex", "Shell"],
// 	},

// 	{
// 		name: "Design Pattern",
// 		description: "Various design patterns implemented in java.",
// 		gitURL: "https://github.com/Waqar-107/Design-Pattern",
// 		tags: ["Java"],
// 	},

// 	{
// 		name: "Tic-Tac-Toe",
// 		description:
// 			"The classic tic-tac-toe game implemented in native android. Min-max algorithm have been used that drives the opponent.",
// 		gitURL: "https://github.com/Waqar-107/Tic-Tac-Toe",
// 		tags: ["Java", "Android"],
// 	},

// 	{
// 		name: "Lost and Survive",
// 		description:
// 			"An app made for IEEE MadC 2017. The app had informations that can be used during travelling, also it gives the nearest police stations and hospitals location. It can send mail in case of emergency.",
// 		gitURL: "https://github.com/Waqar-107/Lost_and_Survive",
// 		tags: ["Java", "Android"],
// 	},

// 	{
// 		name: "Flappy-Ball",
// 		description:
// 			"A game using c and c++. For the gui we used igraphics. It was our term project of level-1, term-1.",
// 		gitURL: "https://github.com/Waqar-107/Flappy-ball",
// 		deployedLink: "https://main.d1as7x06xeg9bv.amplifyapp.com",
// 		tags: ["C", "C++"],
// 	},
// ];

